		...
		function display_sub(p, indent)
			...
			if p.isRoot()
				...
				elseif iscell(p.packspec)
					units_ = p.getPropertyValue('units');
					val = panel.resolveUnits({p.packspec{1} 'mm'}, units_);
					pp = sprintf('%.1f%s', val, units_);
...
% 			invalid = false; Unused
			...
			for a = 1:length(varargin)
				...
				switch key
					...
					case 'm'
						pars.margin = validate_par(str2double(val), arg, 'dimension', 'nonneg');

					case 'i'
						pars.intercolumnspacing = validate_par(str2double(val), arg, 'scalar', 'nonneg');

					case 'c'
						pars.cols = validate_par(str2double(val), arg, 'scalar', 'integer');
					...
					case 'f'
						switch val
							...
							otherwise
								pars.fill = validate_par(str2double(val), arg, 'scalar', [0 1]);
						end
					...
					case 'a'
						switch val
							...
							otherwise
								pars.fill = -validate_par(str2double(val), arg, 'scalar', [0.1 10]);
						end

					case 'w'
						pars.width = validate_par(str2double(val), arg, 'scalar', 'nonneg', [10 Inf]);

					case 'h'
						pars.height = validate_par(str2double(val), arg, 'scalar', 'nonneg', [10 Inf]);
					...
					case 'r'
						...
						switch val
							...
							otherwise
								pars.dpi = validate_par(str2double(val), arg, 'scalar', [75 2400]);
						end
...
%%%% GET TARGET DIMENSIONS (END)
			...
			if is_bitmap
				psz = sz * panel.INCH_PER_MM * pars.dpi;
			...
			% handle smoothing
			if pars.smooth > 1
				psz = sz * pars.smooth * panel.INCH_PER_MM * pars.dpi;
				...
				switch pars.fmt
					case 'png'
						dpm = pars.dpi * panel.INCH_PER_MM * panel.MM_PER_M; 
		...
		function repack(p, packspec)
			...
			if iscell(packspec)
				units_ = p.getPropertyValue('units');
				packspec{1} = panel.resolveUnits({packspec{1} units_}, 'mm');
			end
		...
		function pack(p, varargin)
            ...
            % args must be the same length as nargin because each input
            % argument is must be valid. Otherwise, panel:InvalidArgument
            % will be thrown
...
% 				panel.debugmsg('use of LEGACY interface to pack()', 1);
			...
			for i = 1:length(arg)
				...
				% validate
				validate_packspec(packspec_);
				
				% handle units
				if iscell(packspec_)
					units_ = p.getPropertyValue('units');
					packspec_{1} = panel.resolveUnits({packspec_{1} units_}, 'mm');
				end

				% create a child
				child = panel('pack', p);
				child.packspec = packspec_;
...
% 				figure(p.h_figure);
	...
	methods (Hidden = true)
		...
		function out = le(~, ~) % le(p, q)
			error('panel2:MethodNotImplemented', 'inequality operators are not supported by panel');
		end
		
		function out = lt(~, ~) %  lt(p, q)
			error('panel2:MethodNotImplemented', 'inequality operators are not supported by panel');
		end
				...
				case 2
					...
					switch refs(1).subs
						case {'children' 'ch'}
							cs = p.m_children;
							for c = 1:length(cs)
								cs(c) = subsasgn(cs(c), refs(2:end), value);
							end
						case {'descendants' 'de'}
							cs = p.getPanels('*');
							for c = 1:length(cs)
			...
			switch refs(1).subs
				...
				case 'object'
 				...
 				case {'ch' 'children' 'de' 'descendants' 'fa' 'family'}
					...
					switch refs(1).subs
						case {'children' 'ch'}
							out = cell(1,length(p.m_children));
...
% 			end
			...
			if isempty(context.size_in_mm)
				...
				switch get(p.h_figure, 'Units')
					...
					case 'points'
						points_per_inch = 72;
						context.size_in_mm = context_size / points_per_inch * panel.MM_PER_INCH;
					case 'inches'
						context.size_in_mm = context_size * panel.MM_PER_INCH;
						
					case 'centimeters'
						context.size_in_mm = context_size * 10.0;
						
					case 'pixels'
						context.size_in_mm = context_size / screen_dpi * panel.MM_PER_INCH;
			...
			% it
			margin_ = p.getPropertyValue('margin', 'mm');
			m = margin_([1 3]) / context.size_in_mm(1);
			context.rect = context.rect + [m(1) 0 -sum(m) 0];
			m = margin_([2 4]) / context.size_in_mm(2);
		...
		function recurseComputeLayout(p, context)
			...
			% children
% 			margins = [];  % TODO: resolve situation when relative list is empty list
			...
			rel_list = false(1,length(p.m_children));
            ...
            % TODO: resolve situation when relative list is empty list
            rel_list = find(rel_list);

			% array of actual sizes as fraction of parent (note we
			% only represent the rel_list).
            n_rel_list = length(rel_list);
			zz = zeros(1,n_rel_list);
			sz_phys = zz;
			sz_frac = zz;
			i_stretch = false(1,n_rel_list);
            margins = zeros(n_rel_list+1,n_rel_list);
			...
			for i = 1:n_rel_list
				...
				% get internal margin
				margin_ = c.getPropertyValue('margin', 'mm');
				if p.packdim == 2
					margin_ = margin_([2 4]);
					margin_ = fliplr(margin_); % doclink FLIP_PACKDIM_2 - same reason, here!
				else
					margin_ = margin_([1 3]);
				end
				margins(i:i+1, i) = margin_';
			...
			% sharing space with.
			sz_frac(i_stretch) = (1 - sum(sz_frac)) / sum(i_stretch);
		...
		function setPropertyValue(p, key, value)
			...
			switch key
				case {'margin'}
					...
					if ~isempty(value)
						units_ = p.getPropertyValue('units');
						value = {panel.resolveUnits({value units_}, 'mm') 'mm'};
					end
			end
		...
		function lockClass()
			...
			if isempty(hasLocked)
				...
				if ~panel.isDebug()
					...
					if strcmp(getenv('USERDOMAIN'), 'TINPC')
		...
		function v = resolveUnits(d, units)
			...
			switch d{2}
				...
				case 'cm'
					v = v * 10.0;
				case 'in'
					v = v * panel.MM_PER_INCH;
				case 'pt'
					v = v * panel.PTS_2_INCH * panel.MM_PER_INCH;
				otherwise
			...
			switch units
				...
				case 'cm'
					v = v / 10.0;
				case 'in'
					v = v * panel.INCH_PER_MM;
				case 'pt'
					v = v * panel.INCH_PER_MM * panel.INCH_2_PTS;
				otherwise
